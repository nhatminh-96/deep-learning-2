...
def entre_sortie_reseau(DNN_, input):
    ...
    for layer in DNN_.DNN:
        input = layer.entree_sortie_RBM(input)
        output.append(input)
    # Last layer - softmax classification head
    input = torch.from_numpy(input).to(device)
    softmax_proba = calcul_softmax(DNN_.classification_head, input)
    ...
    with tqdm(total = epochs, unit_scale=True, postfix={'Erreur ':0.0}, ncols=100) as pbar:
        for epoch in range(epochs):
            ...
            for i in range(0, input.shape[0] - batch_size, batch_size):
                ...
                for i in range(DNN_.nb_layers):
                    DNN_.DNN[i].W = DNN_.DNN[i].W - lr*gradients_['W_'+str(i)]
